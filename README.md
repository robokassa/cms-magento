...
### Changelog
= 1.0.2 =
* Исправлено формирование номенклатуры в фискальном чеке при скидках и количестве товара больше 1.
* Для позиций чека теперь передается `cost` без `sum`, чтобы исключить расхождение между суммой позиции и `cost * quantity`.
